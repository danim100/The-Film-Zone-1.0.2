...
Base = 'http://entretenimientoenvideo.blogspot.mx/2016/10/the-film-zone-102.html' #    modificar("la ruta de tus listas")

addon = xbmcaddon.Addon('plugin.video.The Film Zone') #    modificar("el nombre de tu addons")
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if not name in FAV:
                contextMenu.append(('Add to PlayoniptvBase Lists Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)' #    modificar("el nombre de tu addons")
